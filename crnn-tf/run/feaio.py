...
class FeatureIO(object):
  ...
  def sparse_tensor_to_str(self, spares_tensor):
    if not isinstance(spares_tensor, (tf.SparseTensor, tf.SparseTensorValue)):
      raise TypeError('%s should be tf.SparseTensor' % type(spares_tensor))
...
class FeatureReader(FeatureIO):
  ...
  def read_features(tfrecords_path, num_epochs):
    ...
    filename_queue = tf.train.string_input_producer(
        [tfrecords_path], num_epochs=num_epochs)
...
class FeatureIO(object):
  ...
  def __init__(self):
    self.__writer = FeatureWriter()
    self.__reader = FeatureReader()
    return
